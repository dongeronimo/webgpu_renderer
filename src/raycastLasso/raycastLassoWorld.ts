...
import { clearLassos, setCtfHuRange } from "../redux/actions";
...
//Os módulos do ESS vêm de lá mesmo: a ocupação por chunk e o PiP de debug não
//são a técnica em teste aqui, então são REUSADOS, não clonados.
...
// Mundo do RAYCASTER com LASSO DE REMOÇÃO — clone NÃO-DESTRUTIVO do
// RaycastESSWorld (o ESS fica intacto pra A/B de qualidade/velocidade).
// Idêntico a ele (proxy-cube, CTF pré-integrada, gradiente, framebuffer scaling,
// empty-space skipping, PiP de debug dos chunks), MAIS o lasso: o usuário
// desenha um polígono na tela e o que projeta dentro dele, pela câmera daquele
// instante, para de ser renderizado.
//
// A pilha de lassos (com undo/redo) vive no redux; a UI (LassoOverlay/LassoPanel)
// escreve, a VolumeRaycastLassoBehaviour lê, congela a câmera e alimenta o
// material. O ESS continua CORRETO com o lasso ligado sem saber dele: o skip-map
// só pula o que é comprovadamente vazio, e o lasso apenas remove mais coisa —
// ele só não acelera (chunk inteiramente dentro do lasso ainda é marchado).
export class RaycastLassoWorld extends World {
    ...
    async createWorld(perspective: { aspect: number; fovy: number; near: number; far: number; }): Promise<void> {
        ...
        //A pilha de lassos NÃO sobrevive a uma troca de mundo. O polígono está
        //no redux (que sobrevive), mas a matriz da câmera congelada mora no
        //cache da behaviour, que morre com o mundo — sem isso, ao voltar pra cá
        //os lassos antigos seriam re-congelados contra a câmera de AGORA e
        //cortariam no lugar errado, em silêncio. Zerar é o comportamento
        //honesto: o lasso pertence à sessão com aquele volume.
        store.dispatch(clearLassos());
        ...
        //O cérebro: CTF + alpha + gradiente + skip-map + ESS on/off + a pilha de
        //lassos. Recebe o nó da câmera pra poder CONGELAR view+proj no instante
        //em que um lasso novo aparece no redux.
